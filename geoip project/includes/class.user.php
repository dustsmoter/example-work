...
class user
{
    ...
    function create( $data )
    {
    	...
    	{            
    		...
    		$sql = "INSERT INTO user(username, password, email, date_created, status, user_type_id) ".
                   "VALUES ('%s', '%s', '%s', NOW(), '%s', %d)";
    		$this->db->query($sql, $username, $password, $email, $status, $user_type_id);
    ...
    function edit( $post_data )
    {
        if( !empty($post_data) )
        {
            ...
            $sql = "UPDATE user SET username = '%s', password = '%s', email = '%s', status = '%s', user_type_id = %d ".
                   "WHERE id = %d";
            $this->db->query($sql, $username, $password, $email, $status, $user_type_id, $id);
